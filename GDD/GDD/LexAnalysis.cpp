...
namespace Lexer
{
	...
	LEX Analyze(In::IN in, Log::LOG log)
	{
		...
		for (int i = 0; word[i] != NULL; i++, indxLex++)											// проход по лексемам
		{
			...
			FST::FST fstRead(word[i], GRAPH_READ);
			if (FST::execute(fstRead))
			{
				LT::Entry entryLT = writeEntry(entryLT, LEX_READ, indexID++, line);
				LT::Add(lextable, entryLT);
				entryIT = {};
				_mbscpy(entryIT.id, word[i]);
				entryIT.idxfirstLE = indxLex;
				entryIT.idtype = IT::S;
				_mbscpy(entryIT.visibility, emptystr);
				entryIT.iddatatype = READ_TYPE;
				entryIT.value.params.count = READ_PARAMS_CNT;
				entryIT.value.params.types = new IT::IDDATATYPE[READ_PARAMS_CNT];
				for (int k = 0; k < READ_PARAMS_CNT; k++)
					entryIT.value.params.types[k] = IT::READ_PARAMS[k];
				IT::Add(idtable, entryIT);
				entryIT = {};
				findStnFunc = true;
				continue;
