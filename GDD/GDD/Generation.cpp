...
namespace Generator
{
	...
	void Gener::Head()
	{
		...
		out << "includelib D:/¡√“”/GDD/GDD/Debug/Lib.lib\n";
			...
			case LEX_EQUAL:
			{
				...
				while (lexT.table[i].lexema != LEX_SEMICOLON)
				{
					...
					case '@':
					{
						...
						if (flag_callfunc && !flag_link && !flag_measure && !flag_exp && !flag_rand && !flag_comp && !flag_read)
						{
							if (countParm == 1) {
								out << "\tcall " << idT.table[lexT.table[i - countParm].idxTI].id << "\n\tpush eax\n";
